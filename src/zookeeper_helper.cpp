...
{
    assert("Completion data is NULL"&&data);
    static_cast<CAsyncCompletion*>((void*)data)->data_compl(errcode,value,len,stat);
}
...
{
    assert("Completion data is NULL"&&data);
    static_cast<CAsyncCompletion*>((void*)data)->strings_compl(errcode,strings);
}
void async_completion(int errcode,const String_vector *strings, const Stat *stat, const void *data)
{
    assert("Completion data is NULL"&&data);
    static_cast<CAsyncCompletion*>((void*)data)->strings_compl(errcode,strings,stat);
}
...
////////////////////////////////////CAsyncCompletion////////////////////////////////////////
CAsyncCompletion::CAsyncCompletion():m_triggered(false), m_errcode(0)
{
...
void CAsyncCompletion::data_compl(int errcode, const char *value, int len, const Stat *stat)
{
	m_triggered = (true);
	m_errcode = errcode;
	m_stat = *stat;
...
void CAsyncCompletion::strings_compl(int errcode,const String_vector *strings, const Stat *stat)
{
	m_triggered = (true);
	m_errcode = errcode;
	m_stat = *stat;
...
///////////////////////////////////////////////CZookeeperHelper///////////////////////////////////////////////////
CZookeeperHelper::CZookeeperHelper(const std::string& hosts,
		uint16_t timeout,
		uint16_t buffer_length,
		ZooLogLevel log_level,
		bool enable_logfile)
: m_last_errorcode(0), m_timeout(0), m_zk_handle(NULL), m_watcher_init(NULL), m_log_file(NULL)
{
	m_hosts = hosts;
	m_timeout = timeout;
...
bool CZookeeperHelper::connect()
{
	...
	m_zk_handle = zookeeper_init(m_hosts.c_str(), active_watcher, m_timeout, NULL, m_watcher_init, 0);
...
bool CZookeeperHelper::reconncet()
{
	...
	m_watcher_init = new CWatcherAction;
	m_zk_handle = zookeeper_init(m_hosts.c_str(), active_watcher, m_timeout, &m_client_id, m_watcher_init, 0);
...
/////////////
bool CZookeeperHelper::zookeeper_create(const std::string& path, const std::string& value, std::string* path_value, const ACL_vector& acl_entries, int flags)
{
	...
	int errcode =  zoo_create(m_zk_handle, path.c_str(), value_ptr, value_length, &acl_entries, flags, buffer, buffer_length);
...
bool CZookeeperHelper::zookeeper_create(const std::string& path, const std::string& value, CAsyncCompletion* completion, const ACL_vector& acl_entries, int flags)
{
	const char* value_ptr = NULL;
	int         value_length = -1;
	...
	int errcode = zoo_acreate(m_zk_handle, path.c_str(), value_ptr, value_length, &acl_entries, flags, async_completion, completion);
...
bool CZookeeperHelper::zookeeper_set(const std::string& path, const std::string& value, int version)
{
	const char* value_ptr = NULL;
	int         value_length = -1;
	...
	int errcode = zoo_set(m_zk_handle ,path.c_str(), value_ptr, value_length, version);
...
bool CZookeeperHelper::zookeeper_set(const std::string& path, const std::string& value, CAsyncCompletion* completion, int version)
{
	const char* value_ptr = NULL;
	int         value_length = -1;
	...
	int errcode = zoo_aset(m_zk_handle ,path.c_str(), value_ptr, value_length, version, async_completion, completion);
...
bool CZookeeperHelper::zookeeper_delete(const std::string& path, CAsyncCompletion* completion, int version)
{
	int errcode = zoo_adelete(m_zk_handle ,path.c_str(), version, async_completion, completion);
...
bool CZookeeperHelper::zookeeper_exists(const std::string& path, CWatcherAction* watcher_action, Stat* stat)
{
	int errcode = 1;
	...
	{
		errcode = zoo_wexists(m_zk_handle, path.c_str(), active_watcher, watcher_action, stat);
	}
...
bool CZookeeperHelper::zookeeper_exists(const std::string& path, CAsyncCompletion* completion, CWatcherAction* watcher_action)
{
	int errcode = 1;
	if(watcher_action == NULL)
	{
		errcode = zoo_aexists(m_zk_handle, path.c_str(), 0, async_completion, completion);
	}
	else
	{
		errcode = zoo_awexists(m_zk_handle, path.c_str(), active_watcher, watcher_action, async_completion, completion);
	}
...
bool CZookeeperHelper::zookeeper_get(const std::string& path, std::string* value, CWatcherAction* watcher_action, Stat* stat)
{
	...
	if(watcher_action == NULL)
	{
		errcode = zoo_get(m_zk_handle, path.c_str(), 0, buffer, &buffer_length, stat);
	}
	else
	{
		errcode = zoo_wget(m_zk_handle, path.c_str(), active_watcher, watcher_action, buffer, &buffer_length, stat);
	}
...
bool CZookeeperHelper::zookeeper_get(const std::string& path, CAsyncCompletion* completion, CWatcherAction* watcher_action)
{
	int errcode = 1;

	if(watcher_action == NULL)
	{
		errcode = zoo_aget(m_zk_handle, path.c_str(), 0, async_completion, completion);
	}
	else
	{
		errcode = zoo_awget(m_zk_handle, path.c_str(), active_watcher, watcher_action, async_completion, completion);
	}
...
bool CZookeeperHelper::zookeeper_get_children(const std::string& path, std::vector<std::string>* childrens, CWatcherAction* watcher_action, Stat* stat)
{
	int errcode = 1;
	...
	if(watcher_action == NULL)
	{
		errcode = zoo_get_children2(m_zk_handle, path.c_str(), 0, &vec_strings, stat);
	}
	else
	{
		errcode = zoo_wget_children2(m_zk_handle, path.c_str(), active_watcher, watcher_action, &vec_strings, stat);
	}
...
bool CZookeeperHelper::zookeeper_get_children(const std::string& path, CAsyncCompletion* completion, CWatcherAction* watcher_action)
{
	int errcode = 1;

	if(watcher_action == NULL)
	{
		errcode = zoo_aget_children2(m_zk_handle, path.c_str(), 0, async_completion, completion);
	}
	else
	{
		errcode = zoo_awget_children2(m_zk_handle, path.c_str(), active_watcher, watcher_action, async_completion, completion);
	}
...
bool CZookeeperHelper::zookeeper_get_acl(const std::string& path, std::vector<ACL>* acls, Stat* stat)
{
...
bool CZookeeperHelper::zookeeper_get_acl(const std::string& path, CAsyncCompletion* completion)
{
	int errcode = zoo_aget_acl(m_zk_handle, path.c_str(), async_completion, completion);
...
bool CZookeeperHelper::zookeeper_set_acl(const std::string& path, const std::vector<ACL>& acls, int version)
{
	struct ACL_vector vec_acl;
...
bool CZookeeperHelper::zookeeper_set_acl(const std::string& path, const std::vector<ACL>& acls, CAsyncCompletion* completion, int version)
{
	struct ACL_vector vec_acl;
	allocate_ACL_vector(&vec_acl, static_cast<int>(acls.size()));
	...
	int errcode = zoo_aset_acl(m_zk_handle, path.c_str(), version, &vec_acl, async_completion, completion);
    deallocate_ACL_vector(&vec_acl);
...
STOption CZookeeperHelper::zookeeper_create_op_init(const std::string& path, const std::string& value, const ACL_vector& acl_entries, int flags)
{
	STOption op;
...
STOption CZookeeperHelper::zookeeper_set_op_init(const std::string& path, const std::string& value, int version)
{
	STOption op;
...
STOption CZookeeperHelper::zookeeper_delete_op_init(const std::string& path, int version)
{
	STOption op;
...
STOption CZookeeperHelper::zookeeper_check_op_init(const std::string& path, int version)
{
	STOption op;
...
bool CZookeeperHelper::zookeeper_multi(const std::vector<STOption>& options, std::vector<STResult>& results)
{
	...
	for(int i = 0; i < static_cast<int>(options.size()); i++)
	{
		...
		switch (op->type)
		{
			...
			case STOption::Z_ZOO_CREATE :
			{
				...
				zoo_create_op_init(z_op, op->create_op.path.c_str(), value_ptr, value_length, &(op->create_op.acl), op->create_op.flags, new char[m_default_buffer_length], m_default_buffer_length);
				break;
			}
			case STOption::Z_ZOO_SET :
			{
				...
				zoo_set_op_init(z_op, op->set_op.path.c_str(), value_ptr, value_length, op->set_op.version, new Stat);
				break;
			}
			case STOption::Z_ZOO_DELETE :
			{
				zoo_delete_op_init(z_op, op->set_op.path.c_str(),  op->set_op.version);
				break;
			}
			case STOption::Z_ZOO_CHECK :
			{
				zoo_check_op_init(z_op, op->set_op.path.c_str(),  op->set_op.version);
				break;
			}
		}
	}

	int errcode = zoo_multi(m_zk_handle, static_cast<int>(options.size()), ops_arrary, ops_results_array);
...
bool CZookeeperHelper::zookeeper_multi(const std::vector<STOption>& options, CAsyncCompletion* completion, std::vector<STResult>& results)
{
	zoo_op_t ops_arrary[options.size()];
	zoo_op_result_t ops_results_array[options.size()];
	...
	int errcode = zoo_amulti(m_zk_handle, static_cast<int>(options.size()), ops_arrary, ops_results_array, async_completion, completion);
